...
class MFNetSegmentationDataset(Dataset):
    ...
    def _read_split(self):
        ...
        with open(split_path, "r", encoding="utf-8") as file:
            raw_ids = [line.strip() for line in file if line.strip()]
        if not raw_ids:
        ...
        if not samples:
            raise ValueError(
                f"MFNet split file has no usable sample ids after removing "
                f"'_flip' entries: {split_path}"
            )
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            thermal = torch.from_numpy(np.ascontiguousarray(thermal.transpose(2, 0, 1))).float()
        else:
    ...
    def _augment(self, rgb, thermal, label):
        ...
        thermal = self.normalize(thermal)
    ...
    @classmethod
    def _to_tensor(cls, image):
        return torch.from_numpy(np.ascontiguousarray(cls.normalize(image).transpose(2, 0, 1))).float()
