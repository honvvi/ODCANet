...
def parse_args():
    ...
    # Default None: crop/pad back to each image's original HxW (matches test/our Cv).
    # Pass both to use a fixed crop, e.g. --crop_height 480 --crop_width 640.
...
def set_learning_rate(optimizer, base_lr, current_iter, total_iters, warm_iters, lr_power):
    if warm_iters > 0 and current_iter < warm_iters:
        ...
        lr = base_lr * (1.0 - progress) ** lr_power
...
def train_one_epoch(loader, nets, optimizer, device, dataset_name, args, train_cfg, epoch, total_iters):
    ...
    for iteration, batch in enumerate(tqdm(loader, desc=f"Epoch {epoch} Training", leave=False)):
        ...
        if iteration == 0:
            print(
                f"Epoch {epoch}: lr={lr:.8f}, "
                f"ce={float(ce_loss.detach()):.4f}, orth={float(orth_loss.detach()):.4f}"
            )
...
def train(args):
    ...
    val_dataset, val_loader = build_data_loader(
        args.data_root, args.dataset, args.val_split, 1, False, num_workers
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if train_cfg.get("Val_while_train", True) and epoch % int(train_cfg.get("val_step", 1)) == 0:
            val_results = validate(
                val_loader, nets, device, args.dataset, dataset_config
            )
            miou = float(val_results["mIoU"])
            print(
                f"Epoch {epoch}/{epochs}: train_loss={train_loss:.4f}, "
                f"val_loss={val_results['loss']:.4f}, mIoU={miou:.4f}, "
                f"mAcc={val_results['mAccuracy']:.4f}"
            )
            if miou > best_miou:
                best_miou = miou
                best_epoch = epoch
                paths = save_component_weights(nets, output_dir)
                print(f"Saved best checkpoint to {output_dir}: {paths}")
        else:
            print(f"Epoch {epoch}/{epochs}: train_loss={train_loss:.4f}")

    if best_epoch == 0:
        save_component_weights(nets, output_dir)
        best_epoch = epochs
    print(f"Training finished. Best epoch: {best_epoch}, best mIoU: {best_miou:.4f}")
